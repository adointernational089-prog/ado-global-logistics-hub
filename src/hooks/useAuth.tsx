...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const fetchRole = async (userId: string) => {
    ...
    setRole(data as 'admin' | 'staff' | null);
  };
  ...
  const signIn = async (email: string, password: string) => {
    ...
    if (error) {
      return { error: error.message === 'Invalid login credentials' ? 'Access denied. Invalid credentials or account not found.' : error.message };
    }
    return { error: null };
  };
